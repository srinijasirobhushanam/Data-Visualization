setwd("C:\\Users\\student\\Documents\\Data visualization\\Final_Project")
...
# Load The Dataset
sales_data <- fread("BlackFriday.csv")
...
object.size(sales_data) # The data size is 37.6 MB
# B. How many rows and columns are there in the dataset?
dim(sales_data) # 550068 rows and 12 columns
...
str(sales_data) # Text
plot_str(sales_data) # Network Graph
...
sales_data$User_ID <- as.factor(sales_data$User_ID)
sales_data$Product_ID <- as.factor(sales_data$Product_ID)
sales_data$Gender <- as.factor(if_else(sales_data$Gender == 'M', 'Male', 'Female'))
sales_data$Age <- as.factor(sales_data$Age)
sales_data$Occupation <- as.factor(sales_data$Occupation)
sales_data$City_Category <- as.factor(sales_data$City_Category)
sales_data$Stay_In_Current_City_Years <- as.factor(sales_data$Stay_In_Current_City_Years)
sales_data$Marital_Status <- as.factor(if_else(sales_data$Marital_Status == 1, 'Married', 'Single'))
sales_data$Product_Category_1 <- as.integer(sales_data$Product_Category_1)
sales_data$Product_Category_2 <- as.integer(sales_data$Product_Category_2)
sales_data$Product_Category_3 <- as.integer(sales_data$Product_Category_3)
sales_data$Purchase <- as.numeric(sales_data$Purchase)

# Impute the missing values

fit <- rpart(Product_Category_2 ~ User_ID + Product_ID + Age + Gender,
             data = sales_data[!is.na(sales_data$Product_Category_2),], 
             method = "anova")
sales_data$Product_Category_2[is.na(sales_data$Product_Category_2)] <- 
  predict(fit, sales_data[is.na(sales_data$Product_Category_2),])

fit_1 <- rpart(Product_Category_3 ~ User_ID + Product_ID + Age + Gender,
               data = sales_data[!is.na(sales_data$Product_Category_3),], 
               method = "anova")
sales_data$Product_Category_3[is.na(sales_data$Product_Category_3)] <- 
  predict(fit_1, sales_data[is.na(blackf_data$Product_Category_3),])
...
length(unique(sales_data$User_ID))# The store had 5891 customers

# How many items did each customer purchased?
Unique_UserID <- as.data.frame(table(sales_data$User_ID))
...
new_data <- sales_data %>%   
  ...
  summarise_if(funs(mean), Product_Category_1, Product_Category_2, Product_Category_3, Purchase)
